...
class WTAGNNLayer(nn.Module):
    def __init__(self, g, in_feats_node, in_feats_edge, out_feats, activation,
        ...
        self.g = g
        self.weight_node = nn.Parameter(th.Tensor(in_feats_node, out_feats)).to('cuda:0')
        self.weight_edge = nn.Parameter(th.Tensor(in_feats_edge, out_feats)).to('cuda:0')
        if dropout:
            self.dropout = nn.Dropout(p=dropout).to('cuda:0')
        else:
            self.dropout = 0.
        self.node_update = NodeApplyModule(out_feats, activation, bias).to('cuda:0')
        self.edge_update = EdgeApplyModule(out_feats, activation, bias).to('cuda:0')
        self.reset_parameters()
        self.PRINT = False
